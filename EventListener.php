		...
		switch ($block->getID()) {
			...
			case Block::WALL_SIGN:
				...
				if($player->getGamemode() == 1){
					$player->sendMessage("You can't sell in creative");
					$event->setCancelled();
					$this->plugin->getServer()->getLogger()->debug("{$event->getPlayer()->getName()} tried selling in creative mode");
					return;
				}
				...
				if ($itemNum < $saleNum) {
					if($itemNum == 0){
						...
						$this->plugin->getServer()->getLogger()->debug("{$event->getPlayer()->getName()} didn't have enough items to sell");
						return;
					}else{
				...
				//TODO Improve this
				$chest->getInventory()->addItem(clone Item::get((int)$shopInfo['productID'], (int)$shopInfo['productMeta'], (int)$saleNum));
				...
				for ($i = 0; $i < $chest->getSize(); $i++) {
					...
					if ($item->getID() === $pID and $item->getDamage() === $pMeta) {
						if ($item->getCount() < $tmpNum) {
							...
							$player->getInventory()->setItem($i, Item::get($item->getID(), $pMeta, $item->getCount() - $tmpNum));
							break;
						}
					}
				}
				$this->plugin->getServer()->getPluginManager()->getPlugin("MassiveEconomy")->payMoneyToPlayer(($shopInfo['shopOwner']), $price, $player->getName());
				if ($saleNum > 0){
